...
import com.countergame.hscorecounter.databinding.FragmentTennisDoublesGameBinding
...
import com.countergame.hscorecounter.scorecounter.database.Game
import com.countergame.hscorecounter.scorecounter.database.Match
import com.countergame.hscorecounter.scorecounter.database.Set
...
class TennisDoublesGameFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        var p1Name: String? =
            if (arguments?.getString("p1name") != "") arguments?.getString("p1name") else "Player 1"
        var p2Name: String? =
            if (arguments?.getString("p2name") != "") arguments?.getString("p2name") else "Player 2"
        var p12Name: String? =
            if (arguments?.getString("p12name") != "") arguments?.getString("p12name") else "Player 1-2"
        var p22Name: String? =
            if (arguments?.getString("p22name") != "") arguments?.getString("p22name") else "Player 2-2"
        ...
        if (currentGame.finished) {
            ...
            runBlocking {
                if (!currentGame.tiebreak) {
                    dbGameWon(
                        currentGameNumber,
                        points[currentGame.p1Points],
                        points[currentGame.p2Points],
                        currentGame.p1Serving,
                        byPlayer,
                        currentGame.pointHistory.toString(),
                        true,
                        currentSet.p1Games,
                        currentSet.p2Games,
                        dbCurrentSetId
                    ...
                    dbGameWon(
                        currentGameNumber,
                        currentGame.p1Points.toString(),
                        currentGame.p2Points.toString(),
                        currentGame.p1Serving,
                        byPlayer,
                        currentGame.pointHistory.toString(),
                        true,
            ...
            if (currentSet.finished) {
                ...
                var p1color =
                    if (currentSet.p1Games > currentSet.p2Games) resources.getColor(R.color.tblack) else resources.getColor(
                        R.color.tgrey
                    )
                var p2color =
                    if (currentSet.p2Games > currentSet.p1Games) resources.getColor(R.color.tblack) else resources.getColor(
                        R.color.tgrey
                    )
                ...
                if (currentMatch.finished) {
                    ...
                    Toast.makeText(
                        context,
                        "Team ${currentMatch.matchWinner} is the match winner!",
                        Toast.LENGTH_LONG
                    ).show()
                    finished = true
    ...
    private fun updateServingPlayer() {
        binding.p1ServingSymbol.visibility =
            if (currentGame.p1Serving) ImageView.VISIBLE else ImageView.INVISIBLE
        binding.p1ServingSymbol1.visibility =
            if (currentGame.p1Serving) ImageView.VISIBLE else ImageView.INVISIBLE
        binding.p2ServingSymbol.visibility =
            if (currentGame.p1Serving) ImageView.INVISIBLE else ImageView.VISIBLE
        binding.p2ServingSymbol1.visibility =
            if (currentGame.p1Serving) ImageView.INVISIBLE else ImageView.VISIBLE
    }
    ...
    private fun updatePointHistory(byPlayer: Int) {
        ...
        val p1LL =
            (((currentPointHistoryLayout.getChildAt(0) as LinearLayout).getChildAt(0) as LinearLayout).getChildAt(
                0
            ) as TextView)
        p1LL.text = currentSet.p1Games.toString()
        p1LL.visibility = TextView.VISIBLE
        var p1Color = resources.getColor(R.color.tgrey)
        if (currentGame.p1BreakPoint) {
            p1Color = resources.getColor(R.color.tbreak)
        } else if (byPlayer == 1) {
            p1Color = resources.getColor(R.color.tblack)
        }
        p1LL.setTextColor(p1Color)

        val p2LL =
            (((currentPointHistoryLayout.getChildAt(0) as LinearLayout).getChildAt(1) as LinearLayout).getChildAt(
                0
            ) as TextView)
        p2LL.text = currentSet.p2Games.toString()
        p2LL.visibility = TextView.VISIBLE
        var p2Color = resources.getColor(R.color.tgrey)
        if (currentGame.p2BreakPoint) {
            p2Color = resources.getColor(R.color.tbreak)
        } else if (byPlayer == 2) {
            p2Color = resources.getColor(R.color.tblack)
        }
    ...
    private fun createPointView(point: TPoint, byPlayer: Int) {
        ...
        if (currentGame.p1BreakPoint) {
            textColor = resources.getColor(R.color.tbreak)
        } else if (byPlayer == 1) {
            textColor = resources.getColor(R.color.tblack)
        }
    ...
    private fun createPointHistoryLayout(serve: Boolean): HorizontalScrollView {
        ...
        val svparams = LinearLayout.LayoutParams(
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private suspend fun dbNewMatch(
        p1name: String,
        p2name: String,
        p12name: String,
        p22name: String,
        firstserveP1: Boolean,
        mtb: Boolean,
        stw: Int
    ) {
        ...
        dbCurrentMatchId = scoreDao.insertMatch(
            Match(
                "Tennis-Doubles",
                formattedDate,
                p1name,
    ...
    private suspend fun dbUpdateMatch(
        dbMatchId: Long,
        p1Sets: Int,
        p2Sets: Int,
        finished: Boolean
